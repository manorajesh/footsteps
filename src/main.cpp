...
public:
    MoveNetTracker(const std::string& model_path) 
        : env(ORT_LOGGING_LEVEL_WARNING, "MoveNetTracker"),
          session(nullptr) {
        ...
        coreml_flags |= COREML_FLAG_USE_CPU_AND_GPU;  // Use both CPU and GPU/Neural Engine
        ...
        if (status != nullptr) {
            ...
            std::cout << "✓ CoreML (GPU/Neural Engine) acceleration enabled!" << std::endl;
        }
        ...
        for (size_t i = 0; i < input_node_dims.size(); i++) {
            ...
            if (i < input_node_dims.size() - 1) std::cout << ", ";
        }
        std::cout << "]" << std::endl;
    }
    ...
    std::vector<std::array<float, 3>> detectPose(const cv::Mat& frame) {
        ...
        std::vector<int64_t> input_shape = {1, MODEL_INPUT_SIZE, MODEL_INPUT_SIZE, 3};
        ...
        float inference_time = std::chrono::duration_cast<std::chrono::microseconds>(t_end - t_start).count() / 1000.0f;
    ...
    void drawKeypoints(cv::Mat& frame, const std::vector<std::array<float, 3>>& keypoints) {
        ...
        if (keypoints[LEFT_HIP][2] > 0.3 && keypoints[LEFT_KNEE][2] > 0.3) {
            cv::line(frame, 
                    cv::Point(keypoints[LEFT_HIP][1] * width, keypoints[LEFT_HIP][0] * height),
                    cv::Point(keypoints[LEFT_KNEE][1] * width, keypoints[LEFT_KNEE][0] * height),
                    cv::Scalar(255, 255, 0), 2, lineType);
        }
        if (keypoints[LEFT_KNEE][2] > 0.3 && keypoints[LEFT_ANKLE][2] > 0.3) {
            cv::line(frame, 
                    cv::Point(keypoints[LEFT_KNEE][1] * width, keypoints[LEFT_KNEE][0] * height),
                    cv::Point(keypoints[LEFT_ANKLE][1] * width, keypoints[LEFT_ANKLE][0] * height),
                    cv::Scalar(255, 255, 0), 2, lineType);
        }
        if (keypoints[RIGHT_HIP][2] > 0.3 && keypoints[RIGHT_KNEE][2] > 0.3) {
            cv::line(frame, 
                    cv::Point(keypoints[RIGHT_HIP][1] * width, keypoints[RIGHT_HIP][0] * height),
                    cv::Point(keypoints[RIGHT_KNEE][1] * width, keypoints[RIGHT_KNEE][0] * height),
                    cv::Scalar(255, 255, 0), 2, lineType);
        }
        if (keypoints[RIGHT_KNEE][2] > 0.3 && keypoints[RIGHT_ANKLE][2] > 0.3) {
            cv::line(frame, 
                    cv::Point(keypoints[RIGHT_KNEE][1] * width, keypoints[RIGHT_KNEE][0] * height),
                    cv::Point(keypoints[RIGHT_ANKLE][1] * width, keypoints[RIGHT_ANKLE][0] * height),
                    cv::Scalar(255, 255, 0), 2, lineType);
        }
        
        // Draw ankles (2 circles)
        if (keypoints[LEFT_ANKLE][2] > 0.3) {
            cv::circle(frame, cv::Point(keypoints[LEFT_ANKLE][1] * width, keypoints[LEFT_ANKLE][0] * height), 
                      6, cv::Scalar(0, 0, 255), -1, lineType);
        }
        if (keypoints[RIGHT_ANKLE][2] > 0.3) {
            cv::circle(frame, cv::Point(keypoints[RIGHT_ANKLE][1] * width, keypoints[RIGHT_ANKLE][0] * height), 
                      6, cv::Scalar(0, 0, 255), -1, lineType);
        }
    }
};
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (!cap.isOpened()) {
            ...
            std::cerr << "Try a different camera ID with: ./footstep_tracker models/movenet_lightning.onnx <camera_id>" << std::endl;
            return -1;
        }
